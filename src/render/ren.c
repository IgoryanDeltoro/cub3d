int	render_map(void *param)
{
	...
	while (x < WIDTH)
	{
		...
		{
			if (y < draw_start)
				mlx_pixel_put(game->mlx, game->win, x, y,rgb_to_int(game->ceiling_color[0], game->ceiling_color[1], game->ceiling_color[2]));
			else if (y >= draw_start && y <= draw_end)
			{
				...
				mlx_pixel_put(game->mlx, game->win, x, y, rgb_to_int(game->floor_color[0], game->floor_color[1], game->floor_color[2]));
		}
